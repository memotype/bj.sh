...
$1||! $q ]]&&st 1||{ st;k=$(pr);};;[|{)((b++));;]|\})((--b))||{ [[ $q ]]&&return 1;o+=(\
"$c");break;};;:)[[ ! $1&&$q&&$k = "$q" ]]&&return;;,)[[ $1&&$b = 1 ]]&&{ ((n++));[[ $n \
= "$q" ]]&&break;};;esac;done;};for q in "$@" "";do x=1;o=();while rd;do case $c in \
[[:space:]])false;;\")st;;[tfn0-9-])o=();while o+=("$c")&&rd&&[[ $c =~ [-+.0-9Ea-z] ]]
do :;done;;{)co;;[)co 1;;*)return 2;;esac&&{ x=0;break;};done;done;pr;return $x;)
